...
void pipeFunc() { //Function for implementing data transfer using Pipes
    ...
    if (convID == 0) { //To check if the code is in a child process
        // Conv Prozessing
    ...
    } else if (convID > 0) { // To check if the code is in a parent process
        logID = fork(); //Separation of logID after convID into two parallel processes with own generic IDs 
        if (logID == 0) { //To check if the code is in a child process
            // Log Prozessing
            ...
            close(log_to_stat[1]);
        } else if (logID > 0) { // To check if the code is in a parent process
            statID = fork(); //Separation of statID after logID into two parallel processes with own generic IDs
            if (statID == 0) {// To check if the code is in a child process
                // Stat Prozessing
                ...
                close(log_to_stat[0]); //Closing the log_to_stat chanel for reading the information 
            } else if (statID > 0) { // To check if the code is in a parent process
                // Elternprozess
                close(conv_to_log[0]); //Closing the conv_to_log chanel for reading the information 
                close(conv_to_log[1]); //Closing the conv_to_log chanel for writing the information 
                close(log_to_stat[0]); //Closing the log_to_stat chanel for reading the information 
